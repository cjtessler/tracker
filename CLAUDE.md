...
- **Scheduled sync, not live**: `Sync.init()` does no network. Completed-session writes are queued locally but **not** flushed immediately. A scheduler fires once a day at **5:00 PM America/New_York** (DST-safe) and runs `flush → pull → flush`, then hard-reloads the page (`location.reload(true)`) so any pulled rows / UI updates appear without manual intervention. The user can also tap the **"Sync Now"** button in the History footer at any time; manual taps do **not** reload the page. There is no 30 s retry loop, no `window.online` flush, and no startup sync — strict 5 PM only. This trades cross-device freshness for zero background CPU/network during work hours (the device is on overnight).
- **Sync queue** persists in `pedal-tracker-sync-queue`. Items that 4xx three times move to `pedal-tracker-sync-deadletter`. If a scheduled or manual sync fails (offline / 5xx), the queue is preserved and drains on the next 5 PM run or the next Sync Now tap.
...
The version stamp shown in the Settings header is **derived from git**, not edited by hand. `app.js` reads `window.APP_VERSION` from `version.js` (gitignored) which is regenerated by `.githooks/_write-version.sh` from `git describe --tags --always --dirty`. Parallel branches no longer collide on a version constant.

**Cutting a release:** tag the commit with semver — `git tag v1.5.0 && git push --tags`. The next checkout/pull on any machine regenerates `version.js` and the header updates. Use semver: patch for fixes, minor for new behavior, major for breaking UX. Between tags the header shows `v1.5.0-3-gabc123` (3 commits past v1.5.0) which is exactly what you want for "is this the deployed build?"

**One-time setup per clone (dev machines and the Pi):**
```
